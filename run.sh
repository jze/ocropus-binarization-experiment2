...
function checkPrerequisites() {
	...
	if [ $? != 0 ]; then
		echo "ScanTailor not installed? Could not execute 'scantailor-cli'. Check README.md"
		exit 1
	fi
...
function runNetbpm() {
	...
	ocropus-gpageseg -q -n --threshold 0.5 --csminheight 100000 --usegauss work.bin.png 
	recognitionAndError
	echo -e "nlbin+gpageseg bin\t$error" >> gray.csv
	
	ocropus-gpageseg -q -n --gray --threshold 0.5 --csminheight 100000 --usegauss work.nrm.png 
	rm -f work/*.bin.png
	recognitionAndError
